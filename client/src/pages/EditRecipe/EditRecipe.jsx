...
export function EditRecipe() {
  ...
  console.log(state);
  const stateIngredientList = state.ingredients.map((ingredient) => ({
    name: ingredient.ingredient.name,
    price: ingredient.ingredient.price,
    icon: icons[ingredient.ingredient.icon],
    quantity: ingredient.quantity,
    id: ingredient.ingredient.id,
  }));
  console.log(stateIngredientList);

  //será substituido por um estado
  const [recipeName, setRecipeName] = useState(state.name);
  const [recipeIcon, setRecipeIcon] = useState(state.icon);
  ...
  const editRecipe = async (recipe) => {
    try {
      const response = await fetch(
        `http://localhost:3001/recipes/${state.id}`,
        {
          method: "PUT",
          headers: {
      ...
      console.error("Falha ao editar receita: ", error.message);
    }
  };
